...
<div style="float:left" class="form-group {{ $col }} {{ isset($attributes['required']) ? 'required' : '' }} {{ $errors->has($name) ? 'has-error' : 'validated'}}">
    ...
    <div class="input-group">
        ...
        {!! Form::select($name, $values, $selected, array_merge(['class' =>   $errors->has($name) ? 'form-control kt-selectpicker is-invalid' : 'form-control kt-selectpicker',  'multiple', 'placeholder' => trans('general.form.select.field', ['field' => $text])], $attributes)) !!}
        @error($name)
